...
async function Home() {
    ...
    try {
        ...
        const snap = await adminDb.collection("events").where("registrationFormType", "==", "volleyball").limit(1).get();
        if (!snap.empty) {
            const data = snap.docs[0].data();
            slug = data.slug || snap.docs[0].id;
        }
        if (!slug) {
            const featured = await adminDb.collection("events").where("category", "==", "FEATURED_EVENTS").limit(1).get();
            if (!featured.empty) {
    ...
    (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$dist$2f$client$2f$components$2f$navigation$2e$react$2d$server$2e$js__$5b$app$2d$rsc$5d$__$28$ecmascript$29$__["redirect"])(slug ? `/events/${slug}` : "/events");
}
__turbopack_async_result__();
} catch(e) { __turbopack_async_result__(e); } }, false);}),
...
//# sourceMappingURL=%5Broot-of-the-server%5D__e8b92712._.js.map