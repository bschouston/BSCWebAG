...
"[project]/apps/web/src/app/api/admin/rsvps/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(request) {
    ...
    try {
        ...
        let rsvps = [];
        try {
            const rsvpsQuery = await adminDb.collection("event_rsvps").where("eventId", "==", eventId).orderBy("createdAt", "desc").get();
        ...
        const registrationsQuery = await adminDb.collection("events").doc(eventId).collection("event_registrations").orderBy("registeredAt", "desc").get();
        const customRsvps = registrationsQuery.docs.filter((docSnapshot)=>!docSnapshot.data().isDraft).map((docSnapshot)=>{
            const data = docSnapshot.data();
            return {
                id: docSnapshot.id,
                eventId: eventId,
                ...
                user: {
                    ...
                    photoURL: null,
                    skillLevels: {
                        [eventData?.sportId || "volleyball"]: data.playFrequency || "Unknown"
                    }
                },
...
//# sourceMappingURL=%5Broot-of-the-server%5D__9b075994._.js.map