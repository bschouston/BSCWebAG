...
export function WeeklyEventTeamsShowcase({
  ...
  return (
    <section className="space-y-5">
        ...
        <div className="rounded-xl border bg-card p-4 sm:p-5">
          ...
          <ul className="space-y-2">
            {data.unassigned.map((m) => (
              <li
                key={m.userId}
                className="rounded-xl border bg-muted/40 px-3 py-2.5 text-lg font-bold text-[#1a3556] dark:text-foreground sm:text-xl"
              >
                {m.name}
              </li>
            ))}
      ...
      {data.locked ? (
        <p className="text-center text-sm font-medium text-[#8a6d00] dark:text-[#ffd700]">Teams are locked</p>
      ) : null}
